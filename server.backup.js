...
app.use('/webhooks/coinbase', require('express').raw({ type: 'application/json' }));
...
app.post('/withdraw', authenticate, async (req, res) => {
  ...
  // Check user balance
  const userBal = await getBalance(req.user.id);
  if (amount > userBal) return res.status(400).json({ message: 'Insufficient balance' });

  // Deduct balance immediately to prevent double spend
  await deductBalance(req.user.id, amount);

  // Queue withdrawal for signing/broadcast
  const tx = await signAndBroadcast(network, token, to, amount);
  
  res.json({ message: 'Withdrawal initiated', tx_hash: tx });
});
